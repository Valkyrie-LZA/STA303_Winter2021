...
author: "Chief Science Officer Zian Lu; ID: 1004710280"
output:
...
header-includes:    
  ...
  - \usepackage{fancyhdr}
  - \pagestyle{fancy}
  - \fancyhead[LO, LE]{Zian Lu, 1004710280}
...
Vincent Kiger
```{r}
filter(crew_data,position=="Communications Officer")
```
...
```
```{r}
x <- nrow(crew_data %>% filter(sub_division == "Engineering"))
y <- nrow(crew_data %>% filter(str_detect(position, "Engineer")))
y / x
...
week1 <- crew_data %>% 
  filter(week=="1") %>% 
  mutate(duty_shift=fct_relevel(duty_shift,"Gamma","Delta","Beta","Alpha"))
```
...
```{r}
ggplot(data = week1,aes(x=duty_shift,y=productivity,colour=shift_team))+
  geom_boxplot()+
  theme_minimal()+
...
```{r}
crew_data %>% 
  ggplot(aes(y = productivity, x = week,colour=division,group=crew_id)) +
...
```{r, eval=FALSE}
model_3 <- lmer(productivity~ week + starfleet_gpa + perseverance_score+ (1 + week|name) + (1|full_team)+
                  (1 + week|full_team), 
                 data = crew_data)
...
```{r}
complete_comp<-competition_data %>% 
  pivot_longer(-c(crewmember),names_to="plant",values_to="number") %>% 
